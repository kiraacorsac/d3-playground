...
export default function Test() {
  ...
  const axisGenerator = d3.axisRight(scaleAxisPositive).ticks(10);
  ...
  useEffect(() => {
    ...
    rect
      ...
      .on("click", function () {
        ...
        console.log(
          "Is datapoint ",
          d3.select(this).data()[0][0],
          " missing from the list ? ",
          !selectedList.includes(d3.select(this).data()[0][0])
        );
        ...
        console.log(
          "Inside the MouseIn Gate 1, value of selected at end of IF: ",
          selected
        );
      })
      .transition()
      ...
      .duration(function (point) {
        return Math.abs(point[0] * 3000);
      })
      .delay(function (point) {
        return Math.abs(point[0] * 3000);
      })
      .easy(d3.easeElastic);
    ...
    const line = d3
      ...
      .y((point) =>
        point[1] >= 0 ? -scaleChart(point[1]) + 18 : -scaleChart(point[1]) - 8
      )
    ...
    const polarLine = d3
      ...
      .x(
        (d) =>
          (polarArcGenerator.centroid(d)[0] +
            polarArcGenerator.centroid(d)[0] * (d.data[1] / maxAxisAmplitude)) /
          2
      )
      .y(
        (d) =>
          (polarArcGenerator.centroid(d)[1] +
            polarArcGenerator.centroid(d)[1] * (d.data[1] / maxAxisAmplitude)) /
          2
      )
